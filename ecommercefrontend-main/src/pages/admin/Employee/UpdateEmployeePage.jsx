...
const UpdateProductPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [categoriesResponse, singleProductResponse] = await Promise.all([
          ...
          fetch(`${apiUrl}/api/employees/${productId}`),
        ]);
        ...
        console.log("Fetched product data:", singleProductData);
        ...
        if (singleProductData) {
          form.setFieldsValue({
            ...
            colors: singleProductData.colors.length ? singleProductData.colors.join("\n") : "", 
  ...
  const onFinish = async (values) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/api/employees/${productId}`, {
        method: "PUT",
        headers: {
